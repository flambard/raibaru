...
%%--------------------------------------------------------------------
init([]) ->
    {ok, #state{pool = matching_pool:new()}}.
...
handle_call(find_match, From, S = #state{pool = Pool}) ->
    ...
    NewPool =
        case matching_pool:match_player(Pool, From) of
            {no_match, P} -> P;
            {match, Opponent, P} ->
...
handle_cast({match_found, {Player1, Player2}}, S) ->
    demonitor(Player1),
    demonitor(Player2),
    game_sup:start_game(Player1, Player2),
    {noreply, S};
...
handle_info({'DOWN', _Monitor, _Type, Pid, _Info}, S = #state{pool = Pool}) ->
    NewPool = matching_pool:remove_player(Pool, Pid),
